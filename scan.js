...
// Ranges that use the flat Roman-numeral structure (no colour subfolders):
//   images/<range>/I.jpeg   II.jpeg   …   look wise1.jpeg   look wise2.jpeg
// Each Roman numeral file → individual tile.
// Files starting with "look" → merged into one "Look Wise" tile.
...
// (no colour subfolders), named with Roman numerals or "look wise".
//
// Structure:  images/<range>/
//   I.jpeg   II.jpeg   III.jpeg … XXX.jpeg  ← individual tiles (one per file)
//   look wise1.jpeg   look wise2.jpeg        ← grouped into ONE "Look Wise" tile
//
// Rules:
//   • Filename matches a Roman numeral (I–L) → individual stone tile
//   • Filename starts with "look" (case-insensitive) → merged into one "Look Wise" tile
//   • Any other filename → individual tile using the filename as display name
// ══════════════════════════════════════════════════════
function scanRomanNumeralRange(rangeFolder) {
  ...
  // Roman numeral pattern: matches I, II, III, IV … up to L (50)
  ...
  for (const file of files) {
    ...
    const noExt   = file.slice(0, file.length - ext.length);
    const relPath = `images/${rangeFolder}/${file}`;

    if (noExt.toLowerCase().startsWith('look')) {
      // Collect for the grouped "Look Wise" tile
      if (!lookSlab) lookSlab = relPath;
      lookImages.push({ src: relPath, tag: 'Look Wise' });
      continue;
    }

    const trimmed = noExt.trim();
    const isRoman = ROMAN_RE.test(trimmed);
    const displayName = isRoman
      ? trimmed.toUpperCase() + ' — ' + rangeLabel
      : toTitle(trimmed) + ' ' + rangeLabel;

    stones.push({
      id:         `${canonicalSlug}__${trimmed.toUpperCase()}`,
      name:       displayName,
      range:      canonicalSlug,
      rangeLabel: rangeLabel,
      colour:     'Multi',
  ...
  // Add the grouped "Look Wise" tile if any look-wise images exist
  if (lookImages.length) {
    stones.push({
      id:         `${canonicalSlug}__look-wise`,
      name:       'Look Wise — ' + rangeLabel,
      range:      canonicalSlug,
      rangeLabel: rangeLabel,
      colour:     'Multi',
      slab:       lookSlab,
...
function buildCatalog() {
  ...
  for (const rangeFolder of rangeFolders) {
    ...
    // ── Special: Roman-numeral flat ranges (indian-italian-marble, travertin, …) ──
