...
const SKIP_RANGES = new Set(['marble-carvings', 'indian-italian-marble']);
...
// ══════════════════════════════════════════════════════
// INDIAN ITALIAN MARBLE SCANNER
//
// Structure:  images/indian-italian-marble/
...
//   • Filename starts with "look" (case-insensitive) → merge into one "Look Wise" tile
// ══════════════════════════════════════════════════════
function scanIndianItalianMarble(rangeFolder) {
  const dir = path.join(IMAGES_DIR, rangeFolder);
  if (!fs.existsSync(dir) || !fs.statSync(dir).isDirectory()) return [];

  // Roman numeral pattern: I, II, III, IV … up to about L (50)
  // Files like "I.jpeg", "XIX.jpeg", "XXVIII.jpeg"
  ...
  const stones     = [];
  const lookImages = [];  // collects all "look wise" images for the grouped tile
  let   lookSlab   = null;

  for (const file of files) {
    const ext   = path.extname(file);
    const noExt = file.slice(0, file.length - ext.length); // e.g. "I", "look wise1"
    const relPath = `images/${rangeFolder}/${file}`;

    const isLookWise = noExt.toLowerCase().startsWith('look');
    ...
    // Check if it's a Roman numeral filename
    const trimmed = noExt.trim();
    if (!ROMAN_RE.test(trimmed)) {
      // Unknown file — treat as individual stone using filename as display name
      stones.push({
        id:         `${rangeFolder}__${trimmed}`.replace(/[\s]+/g, '-'),
        name:       toTitle(trimmed) + ' Indian Italian Marble',
        range:      'indian-italian-marble',
        rangeLabel: 'Indian Italian Marble',
        colour:     'Multi',
        slab:       relPath,
    ...
    // Roman numeral → individual tile
    const displayName = trimmed.toUpperCase() + ' — Indian Italian Marble';
    stones.push({
      id:         `${rangeFolder}__${trimmed.toUpperCase()}`,
      name:       displayName,
      range:      'indian-italian-marble',
      rangeLabel: 'Indian Italian Marble',
      colour:     'Multi',
  ...
  if (lookImages.length) {
    stones.push({
      id:         `${rangeFolder}__look-wise`,
      name:       'Look Wise — Indian Italian Marble',
      range:      'indian-italian-marble',
      rangeLabel: 'Indian Italian Marble',
      colour:     'Multi',
      slab:       lookSlab,
...
function buildCatalog() {
  ...
  for (const rangeFolder of rangeFolders) {
    ...
    // ── Special: indian-italian-marble ──
    if (rangeSlug === 'indian-italian-marble') {
      const iiStones = scanIndianItalianMarble(rangeFolder);
      for (const s of iiStones) {
        allStones.push(s);
        console.log(`  ✓  [indian-italian] ${s.name.padEnd(36)} [${s.images.map(i=>i.tag).join(' + ')}]`);
      }
      continue;
    }
