#!/usr/bin/env node
/**
 * HARGOBIND MARBLE — CATALOG SCANNER  v6
 *
 * FOUR folder structures handled automatically:
 *
 * STANDARD (2-level):
 *   images/<range>/<colour>/
 *     green.jpg          → Slab
 ...
 *
 * CARVINGS (special — named sub-folders, slab/design naming):
 *   images/marble-carvings/
 *     flutes/
 *       slab.jpg  slab1.jpg  design.jpg  design1.jpg
 *     straight-lines/
 *       slab1.jpg  design1.jpg
 *   → One tile per sub-folder; tile name = prettified folder name.
 *
 * HANDICRAFTS (special — named sub-folders, any image names):
 *   images/marble-handicrafts/
 *     vase/
 ...
 *     table-top/
 *       …
 *   → One tile per sub-folder on the Handicrafts page.
 *     Clicking a tile opens a full-page photo grid for that category.
 *     First image = tile thumbnail.
 *
 * Run: node scan.js
 */
...
const RANGE_LABELS = {
  ...
  'indian-marble':'Indian Marble','satwario':'Statuario','vietnam':'Vietnam',
  'travertine':'Travertine','travertine':'Travertine','limestone':'Limestone',
  'premium-luxury-marble':'Premium Luxury Marble',
  'indian-italian-marble':'Indian Italian Marble',
};
...
function getRangeSlug(folderName) {
  ...
  // Partial match (e.g. "travertine" → "travertine")
...
const SKIP_RANGES = new Set(['marble-carvings', 'marble-handicrafts', 'indian-italian-marble', 'travertine', 'travertine']);

// Ranges that use the flat Roman-numeral / prefix-grouped structure
// (images sit directly in the range folder, no colour subfolders)
const ROMAN_NUMERAL_RANGES = new Set(['indian-italian-marble', 'travertine', 'travertine']);
...
function scanCarvings(rangeFolder) {
  ...
  subFolders.forEach(subFolder => {
    ...
    // Tile name = Roman numeral based on folder number, or folder name if not numeric
    const folderNum = parseInt(subFolder, 10);
    const roman     = !isNaN(folderNum) ? toRoman(folderNum) : toTitle(subFolder);
    const name      = roman;
...
//   images/marble-handicrafts/
//     vase/
//       i.jpg   ii.jpg   iii.jpg   …   (any image names, or roman numerals)
//     bathroom-accessories/
//       i.jpg   ii.jpg   …
//     table-top/
//       …
//
// Each sub-folder = one category tile on the Handicrafts page.
// First image in the folder = tile thumbnail.
// All images = the category gallery (detail page).
// Tag is derived from the filename (no extension), capitalised.
// ══════════════════════════════════════════════════════
function scanHandicrafts(rangeFolder) {
  const dir = path.join(IMAGES_DIR, rangeFolder);
  if (!fs.existsSync(dir)) return [];
  ...
  subFolders.forEach(subFolder => {
    ...
    const images = files.map(file => {
      ...
      const noExt   = file.slice(0, file.length - ext.length).trim();
      const relPath = `images/${rangeFolder}/${subFolder}/${file}`;
      // Tag = capitalised filename (strip extension)
      const tag = toTitle(noExt.replace(/[-_]+/g, ' ')) || 'Photo';
      return { src: relPath, tag };
    });

    const categoryName = toTitle(subFolder.replace(/[-_]+/g, ' '));
...
function scanColourFolder_standard(rangeFolder, colourFolder, dirPath, rangeSlug) {
  ...
  const stones = [];
  let stoneIndex = 0; // for Roman numeral labelling of numbered stones
  ...
  for (const [base, g] of groups) {
    ...
    let displayName;
    // Strip colour prefix to get remainder
    const remainder = baseLower.startsWith(colourLower)
      ? baseLower.slice(colourLower.length).trim()
      : null;

    const isNumbered = remainder !== null && (remainder === '' || /^\d+$/.test(remainder));

    if (isNumbered) {
      // Numbered pattern: use Roman numeral as tile name
      // e.g. green1 → "Green Onyx I", green2 → "Green Onyx II"
      const roman = toRoman(remainder ? parseInt(remainder) : stoneIndex);
      displayName = toTitle(colourFolder) + ' ' + rangeLabel + ' ' + roman;
    } else {
      // Named pattern: use the base name directly as the tile name
      displayName = toTitle(base);
    }
...
// ══════════════════════════════════════════════════════
// Ranges listed here give each IMAGE FILE its own tile (not each sub-folder)
const INDIVIDUAL_TILE_RANGES = new Set([]);
...
function scanColourFolder_variety(rangeFolder, colourFolder, dirPath, subFolders, rangeSlug) {
  ...
  for (const varietyFolder of subFolders) {
    ...
    // ── INDIVIDUAL TILE MODE ──
    // Each image file → its own tile. Tile name = filename (prettified).
    if (INDIVIDUAL_TILE_RANGES.has(slug)) {
      files.forEach(file => {
        const relPath = `images/${rangeFolder}/${colourFolder}/${varietyFolder}/${file}`;
        const ext     = path.extname(file);
        const baseName = file.slice(0, file.length - ext.length);
        const tileName = toTitle(baseName);

        stones.push({
          id:         `${slug}__${colourFolder}__${varietyFolder}__${baseName}`.replace(/[\s]+/g, '-'),
          name:       tileName,
          range:      slug,
          rangeLabel: rangeLabel,
          colour:     colour,
          slab:       relPath,
          images:     [{ src: relPath, tag: 'Slab' }],
          individualTile: true,
          subCategory: toTitle(varietyFolder),  // e.g. "Banswara White"
        });
      });
      continue; // skip the grouped-stone logic below
    }

    // ── GROUPED MODE (default — all other ranges) ──
    const images  = [];
    let slabPath  = null;
...
// ══════════════════════════════════════════════════════
// ROMAN-NUMERAL FLAT RANGE SCANNER
//
// Used for ranges whose images sit directly in the range folder
// (no colour subfolders): travertine, indian-italian-marble, etc.
//
// Structure:  images/<range>/
//   I.jpeg   II.jpeg   … XXX.jpeg        <- Roman numeral  -> individual tile each
//   look wise1.jpeg  look wise2.jpeg     <- word + digits  -> ONE "Look Wise" tile
//   Titanium1.jpeg   Titanium2.jpeg      <- word + digits  -> ONE "Titanium" tile
//
// Grouping rules:
//   1. Pure Roman numeral filename        -> individual tile
//   2. Word-prefix + trailing digits      -> all files sharing that prefix
//      are merged into ONE tile
//   3. Anything else                      -> individual tile
// ══════════════════════════════════════════════════════
function scanRomanNumeralRange(rangeFolder) {
  const canonicalSlug = getRangeSlug(rangeFolder);
  const rangeLabel    = RANGE_LABELS[canonicalSlug] || toTitle(rangeFolder);

  const dir = path.join(IMAGES_DIR, rangeFolder);
  if (!fs.existsSync(dir) || !fs.statSync(dir).isDirectory()) return [];

  // Pure Roman numeral: I, II, III, IV up to L (50)
  const ROMAN_RE = /^(XL|XC|L?X{0,3})(IX|IV|V?I{0,3})$/i;

  const files = fs.readdirSync(dir).filter(isImage).sort();
  if (!files.length) return [];

  // Returns the word-prefix if the string ends in digits, otherwise null.
  // "Titanium3" -> "titanium"  "look wise1" -> "look wise"  "XIV" -> null
  function wordPrefix(str) {
    const lower    = str.toLowerCase().trim();
    const stripped = lower.replace(/\s*\d+$/, '').trim();
    return (stripped !== lower && stripped !== '') ? stripped : null;
  }
  ...
  const classified = files.map(file => {
    const ext     = path.extname(file);
    const noExt   = file.slice(0, file.length - ext.length).trim();
    const relPath = `images/${rangeFolder}/${file}`;
    if (ROMAN_RE.test(noExt)) {
      return { type: 'roman', roman: noExt.toUpperCase(), relPath };
    }
    const prefix = wordPrefix(noExt);
    if (prefix) {
      return { type: 'group', prefix, relPath, noExt };
    }
    return { type: 'single', noExt, relPath };
  });

  // ── Pass 2: build group buckets ──
  const groupBuckets = new Map(); // lowercase prefix -> { slab, images[] }
  classified.forEach(f => {
    if (f.type !== 'group') return;
    if (!groupBuckets.has(f.prefix)) {
      groupBuckets.set(f.prefix, { slab: null, images: [] });
    }
    const g = groupBuckets.get(f.prefix);
    if (!g.slab) g.slab = f.relPath;
    g.images.push({ src: f.relPath, tag: toTitle(f.prefix) });
  });

  // ── Pass 3: emit stones in file order ──
  const stones = [];
  ...
  classified.forEach(f => {
    if (f.type === 'roman') {
      stones.push({
        id:         `${canonicalSlug}__${f.roman}`,
        name:       f.roman + ' — ' + rangeLabel,
        range:      canonicalSlug,
        rangeLabel: rangeLabel,
        colour:     'Multi',
        slab:       f.relPath,
        images:     [{ src: f.relPath, tag: 'Slab' }],
      });

    } else if (f.type === 'group') {
      if (emittedGroups.has(f.prefix)) return; // already emitted this group
      emittedGroups.add(f.prefix);
      const g          = groupBuckets.get(f.prefix);
      const groupName  = toTitle(f.prefix);
      const slugPrefix = f.prefix.replace(/\s+/g, '-');
      stones.push({
        id:         `${canonicalSlug}__${slugPrefix}`,
        name:       groupName + ' — ' + rangeLabel,
        range:      canonicalSlug,
        rangeLabel: rangeLabel,
        colour:     'Multi',
        slab:       g.slab,
      ...
      // Single / unknown file -> individual tile
      const slugName = f.noExt.replace(/\s+/g, '-');
      stones.push({
        id:         `${canonicalSlug}__${slugName}`,
        name:       toTitle(f.noExt) + ' — ' + rangeLabel,
        range:      canonicalSlug,
        rangeLabel: rangeLabel,
        colour:     'Multi',
        slab:       f.relPath,
...
function buildCatalog() {
  ...
  for (const rangeFolder of rangeFolders) {
    ...
    // ── Special: marble-handicrafts ──
    if (rangeSlug === 'marble-handicrafts') {
      const handicrafts = scanHandicrafts(rangeFolder);
      for (const s of handicrafts) {
        allStones.push(s);
        console.log(`  ✓  [handicraft] ${s.name.padEnd(24)} [${s.images.length} image${s.images.length!==1?'s':''}]`);
      }
      continue;
    }

    // ── Special: Roman-numeral flat ranges (travertine, indian-italian-marble, …) ──
    if (ROMAN_NUMERAL_RANGES.has(rangeSlug)) {
      const rnStones = scanRomanNumeralRange(rangeFolder);
      for (const s of rnStones) {
        allStones.push(s);
        console.log(`  ✓  [roman]  ${s.rangeLabel.padEnd(22)} ${s.name.padEnd(36)} [${s.images.map(i=>i.tag).join(' + ')}]`);
      }
      continue;
    }

    // Skip any other special folders
